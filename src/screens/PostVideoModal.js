...
export default function PostVideoModal(props) {
  ...
  const handleDelete = () => {
    ...
    const postVideoRef = firebase
      .storage()
      .ref()
      .child(`videos/, ${videoFile}`);
    const postThumbnailRef = firebase
      .storage()
      .ref()
      .child(`images/, ${thumbnailFile}`);
    const postRef = db
  ...
  return <View style={styles.postModal}>{modal}</View>;
}
